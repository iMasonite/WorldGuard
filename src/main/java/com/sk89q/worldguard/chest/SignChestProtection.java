...
public class SignChestProtection implements ChestProtection {
	...
	public boolean isProtected(Block block, Player player) {
		...
		else if (block.getTypeId() == BlockID.SIGN_POST) return isProtectedSignAndChestBinary(block, player);
		else {
	...
	private Boolean isProtectedSign(Sign sign, Player player) {
		if (sign.getLine(0).equalsIgnoreCase("[Lock]")) {
			...
			if (name.equalsIgnoreCase(sign.getLine(1).trim()) || name.equalsIgnoreCase(sign.getLine(2).trim()) || name.equalsIgnoreCase(sign.getLine(3).trim())) return false;
	...
	private Boolean isProtectedSignAndChest(Block block, Player player) {
		if (!isChest(block.getRelative(0, 1, 0).getTypeId())) return null;
	...
	public boolean isChest(int type) {
		return type == BlockID.CHEST || type == BlockID.DISPENSER || type == BlockID.FURNACE || type == BlockID.BURNING_FURNACE;
	}
}
