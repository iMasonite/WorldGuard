...
/** Represents a set of regions for a particular point or area and the rules that are represented by
 * that set. An instance of this can be used to query the value of a flag or check if a player can
 * build in the respective region or point. This object contains the list of applicable regions and
 * so the expensive search of regions that are in the desired area has already been completed.
 * 
 * @author sk89q */
public class ApplicableRegionSet implements Iterable<ProtectedRegion> {
	...
	public boolean canUse(LocalPlayer player) {
		return !allows(DefaultFlag.USE, player) && !canBuild(player);
	}
	...
	public boolean allows(StateFlag flag) {
		if (flag == DefaultFlag.BUILD) throw new IllegalArgumentException("Can't use build flag with allows()");
	...
	/** Indicates whether a player is an owner or member of all regions in this set.
	 * 
	 * @param player player
	...
	private boolean internalGetState(StateFlag flag, LocalPlayer player, LocalPlayer groupPlayer) {
		...
		for (ProtectedRegion region : applicable) {
			...
			if (player != null && region.getFlag(DefaultFlag.PASSTHROUGH) == State.ALLOW) {
				continue;
			}
	...
	/** Gets the value of a flag. Do not use this for state flags (use
	 * {@link #allows(StateFlag, LocalPlayer)} for that).
	 * 
	 * @param flag flag to check
	...
	public <T extends Flag<V>, V> V getFlag(T flag, LocalPlayer groupPlayer) {
		/*
		 * if (flag instanceof StateFlag) { throw new
		 * IllegalArgumentException("Cannot use StateFlag with getFlag()"); }
		 */
