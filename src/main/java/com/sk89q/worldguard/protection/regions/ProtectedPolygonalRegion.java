...
public class ProtectedPolygonalRegion extends ProtectedRegion {
	...
	public boolean contains(Vector pt) {
		...
		if (targetX < min.getBlockX() || targetX > max.getBlockX() || targetZ < min.getBlockZ() || targetZ > max.getBlockZ()) return false;
	...
	public int volume() {
		...
		// TODO: Fix this
		/*
		 * int numPoints = points.size(); if (numPoints < 3) { return 0; } double area = 0; int xa, z1,
		 * z2; for (int i = 0; i < numPoints; i++) { xa = points.get(i).getBlockX(); //za =
		 * points.get(i).getBlockZ(); if (points.get(i + 1) == null) { z1 = points.get(0).getBlockZ(); }
		 * else { z1 = points.get(i + 1).getBlockZ(); } if (points.get(i - 1) == null) { z2 =
		 * points.get(numPoints - 1).getBlockZ(); } else { z2 = points.get(i - 1).getBlockZ(); } area =
		 * area + (xa * (z1 - z2)); } xa = points.get(0).getBlockX(); //za = points.get(0).getBlockZ();
