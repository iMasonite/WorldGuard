...
public abstract class ProtectedRegion implements Comparable<ProtectedRegion> {
	...
	public void setParent(ProtectedRegion parent) throws CircularInheritanceException {
		...
		while (p != null) {
			if (p == this) throw new CircularInheritanceException();
			p = p.getParent();
		}
	...
	public boolean isOwner(String playerName) {
		if (owners.contains(playerName)) return true;
		
		ProtectedRegion curParent = getParent();
		while (curParent != null) {
			if (curParent.getOwners().contains(playerName)) return true;
	...
	public boolean isMember(String playerName) {
		...
		ProtectedRegion curParent = getParent();
		while (curParent != null) {
			if (curParent.getMembers().contains(playerName)) return true;
	...
	protected boolean intersectsEdges(ProtectedRegion region) {
		...
		for (BlockVector2D aPts1 : pts1) {
			for (BlockVector2D aPts2 : pts2) {
				...
				if (line1.intersectsLine(lastPt2.getBlockX(), lastPt2.getBlockZ(), aPts2.getBlockX(), aPts2.getBlockZ())) return true;
				lastPt2 = aPts2;
			}
			lastPt1 = aPts1;
		}
	...
	public boolean equals(Object obj) {
		if (!(obj instanceof ProtectedRegion)) return false;
		
		ProtectedRegion other = (ProtectedRegion) obj;
		return other.getId().equals(getId());
	}
	
	/** Thrown when setting a curParent would create a circular inheritance situation. */
