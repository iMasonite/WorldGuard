...
/** A very simple implementation of the region manager that uses a flat list and iterates through the
 * list to identify applicable regions. This method is not very efficient.
 * 
 * @author sk89q */
public class FlatRegionManager extends RegionManager {
	 ...
	 * @return */
	/*
	 * @Override public ApplicableRegionSet getApplicableRegions(ProtectedRegion checkRegion) {
	 * List<ProtectedRegion> appRegions = new ArrayList<ProtectedRegion>();
	 * appRegions.addAll(regions.values()); List<ProtectedRegion> intersectRegions; try {
	 * intersectRegions = checkRegion.getIntersectingRegions(appRegions); } catch (Exception e) {
	 * intersectRegions = new ArrayList<ProtectedRegion>(); } return new
	 * ApplicableRegionSet(intersectRegions, regions.get("__global__")); }
	 */
