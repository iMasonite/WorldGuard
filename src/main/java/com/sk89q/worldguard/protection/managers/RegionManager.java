...
/** An abstract class for getting, setting, and looking up regions. The most simple implementation
 * uses a flat list and iterates through the entire list to look for applicable regions, but a more
 * complicated (and more efficient) implementation may use space partitioning techniques.
 * 
 * @author sk89q */
public abstract class RegionManager {
	...
	/** Load the list of regions. If the regions do not load properly, then the existing list should be
	 * used (as stored previously).
	 * 
	...
	/** Get a map of protected regions. Use one of the region manager methods if possible if working
	 * with regions.
	 * 
	 * @return map of regions, with keys being region IDs (lowercase) */
	public abstract Map<String, ProtectedRegion> getRegions();
	
	/** Set a list of protected regions. Keys should be lowercase in the given map fo regions.
	 * 
	 * @param regions map of regions */
	public abstract void setRegions(Map<String, ProtectedRegion> regions);
	
	/** Adds a region. If a region by the given name already exists, then the existing region will be
	 * replaced.
	 * 
	...
	/** Get an object for a point for rules to be applied with. Use this in order to query for flag
	 * data or membership data for a given point.
	 * 
	...
	/** Get an object for a point for rules to be applied with. This gets a set for the given reason.
	 * 
	 * @param region region
	 * @return regino set */
	...
	/** Returns true if the provided region overlaps with any other region that is not owned by the
	 * player.
	 * 
