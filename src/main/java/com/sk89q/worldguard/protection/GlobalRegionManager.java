...
/** This class keeps track of region information for every world. It loads world region information
 * as needed.
 * 
 * @author sk89q
 * @author Redecouverte */
public class GlobalRegionManager {
	...
	/** Stores the list of modification dates for the world files. This allows WorldGuard to reload
	 * files as needed. */
	...
	public RegionManager create(World world) {
		...
		try {
			...
			if (plugin.getGlobalStateManager().get(world).summaryOnStart) {
				plugin.getLogger().info(manager.getRegions().size() + " regions loaded for '" + name + "'");
			}
			...
			plugin.getLogger().log(Level.SEVERE, "Error loading regions for world \"" + name + "\": " + e.toString() + "\n\t" + e.getMessage());
			e.printStackTrace();
		}
	...
	public boolean canBuild(Player player, Location loc) {
		...
		if (!worldConfig.useRegions) return true;
		...
		if (!hasBypass(player, world)) {
			...
			if (!mgr.getApplicableRegions(BukkitUtil.toVector(loc)).canBuild(localPlayer)) return false;
		}
	...
	public boolean canConstruct(Player player, Location loc) {
		...
		if (!hasBypass(player, world)) {
			...
			if (!applicableRegions.canBuild(localPlayer)) return false;
			if (!applicableRegions.canConstruct(localPlayer)) return false;
		}
	 ...
	 * 
	 * @see #allows(com.sk89q.worldguard.protection.flags.StateFlag, org.bukkit.Location,
	 ...
	 * @param flag The flag to check
	 * @param loc The location to check the flag at
	 * @param player The player to check for the flag's
	 *        {@link com.sk89q.worldguard.protection.flags.RegionGroup}
	 * @return Whether the flag is allowed */
