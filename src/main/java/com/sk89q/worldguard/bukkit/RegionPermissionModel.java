...
public class RegionPermissionModel extends AbstractPermissionModel {
	...
	public boolean maySetParent(ProtectedRegion child, ProtectedRegion parent) {
		return hasPatternPermission("setparent", child) && (parent == null || hasPatternPermission("setparent", parent));
	}
	...
	public boolean mayList(String targetPlayer) {
		if (targetPlayer == null) return mayList();
		
		if (targetPlayer.equalsIgnoreCase(getSender().getName())) return hasPluginPermission("region.list.own");
		else return mayList();
	}
	...
	public boolean maySetFlag(ProtectedRegion region, Flag<?> flag) {
		...
		return hasPatternPermission("flag.flags." + flag.getName().toLowerCase(), region);
	}
	
	/** Checks to see if the given sender has permission to modify the given region using the region
	 * permission pattern.
	 * 
	 * @param perm the name of the node
	 * @param region the region */
	private boolean hasPatternPermission(String perm, ProtectedRegion region) {
		if (!(getSender() instanceof Player)) return true; // Non-players (i.e. console, command blocks,
																												// etc.) have full power
		...
		if (region.isOwner(localPlayer)) return hasPluginPermission("region." + perm + ".own." + idLower) || hasPluginPermission("region." + perm + ".member." + idLower);
		else if (region.isMember(localPlayer)) return hasPluginPermission("region." + perm + ".member." + idLower);
		else {
