...
public class WorldGuardBlockListener implements Listener {
	...
	public void onBlockBreak(BlockBreakEvent event) {
		...
		if (!plugin.getGlobalRegionManager().canBuild(player, event.getBlock()) || !plugin.getGlobalRegionManager().canConstruct(player, event.getBlock())) {
		...
		if (wcfg.getBlacklist() != null) {
			if (!wcfg.getBlacklist().check(new BlockBreakBlacklistEvent(plugin.wrapPlayer(player), toVector(event.getBlock()), event.getBlock().getTypeId()), false, false)) {
				event.setCancelled(true);
				return;
			}
			
			if (!wcfg.getBlacklist().check(new DestroyWithBlacklistEvent(plugin.wrapPlayer(player), toVector(event.getBlock()), player.getItemInHand().getTypeId()), false, false)) {
				event.setCancelled(true);
				return;
			}
		}
	...
	public void onBlockFromTo(BlockFromToEvent event) {
		...
		/*
		 * if (plugin.classicWater && isWater) { int blockBelow = blockFrom.getRelative(0, -1,
		 * 0).getTypeId(); if (blockBelow != 0 && blockBelow != 8 && blockBelow != 9) {
		 * blockFrom.setTypeId(9); if (blockTo.getTypeId() == 0) { blockTo.setTypeId(9); } return; } }
		 */
		...
		if (wcfg.highFreqFlags && isWater && !plugin.getGlobalRegionManager().allows(DefaultFlag.WATER_FLOW, blockFrom.getLocation())) {
			event.setCancelled(true);
			return;
		}
		
		if (wcfg.highFreqFlags && isLava && !plugin.getGlobalRegionManager().allows(DefaultFlag.LAVA_FLOW, blockFrom.getLocation())) {
			event.setCancelled(true);
			return;
		}
	...
	public void onBlockIgnite(BlockIgniteEvent event) {
		...
		if (wcfg.blockLighter && (cause == IgniteCause.FLINT_AND_STEEL || cause == IgniteCause.FIREBALL) && event.getPlayer() != null && !plugin.hasPermission(event.getPlayer(), "worldguard.override.lighter")) {
			event.setCancelled(true);
			return;
		}
		...
		if (wcfg.useRegions) {
			...
			if (player != null && !plugin.getGlobalRegionManager().hasBypass(player, world)) {
				...
				if (cause == IgniteCause.FLINT_AND_STEEL || cause == IgniteCause.FIREBALL) {
					if (!set.allows(DefaultFlag.LIGHTER) && !set.canBuild(localPlayer) && !plugin.hasPermission(player, "worldguard.override.lighter")) {
						event.setCancelled(true);
						return;
					}
				}
	...
	public void onBlockPlace(BlockPlaceEvent event) {
		...
		if (wcfg.useRegions) {
			...
			if (!plugin.getGlobalRegionManager().canBuild(player, location) || !plugin.getGlobalRegionManager().canConstruct(player, location)) {
				player.sendMessage(ChatColor.DARK_RED + "You don't have permission for this area.");
				event.setCancelled(true);
				return;
			}
		...
		if (wcfg.getBlacklist() != null) {
			if (!wcfg.getBlacklist().check(new BlockPlaceBlacklistEvent(plugin.wrapPlayer(player), toVector(blockPlaced), blockPlaced.getTypeId()), false, false)) {
				event.setCancelled(true);
				return;
			}
		}
		...
		if (wcfg.simulateSponge && blockPlaced.getTypeId() == 19) {
			if (wcfg.redstoneSponges && blockPlaced.isBlockIndirectlyPowered()) return;
	...
	public void onSignChange(SignChangeEvent event) {
		...
		if (wcfg.signChestProtection) {
			if (event.getLine(0).equalsIgnoreCase("[Lock]")) {
				...
				if (below == BlockID.TNT || below == BlockID.SAND || below == BlockID.GRAVEL || below == BlockID.SIGN_POST) {
	...
	public void onBlockPistonRetract(BlockPistonRetractEvent event) {
		...
		if (wcfg.useRegions && event.isSticky()) {
			if (!(plugin.getGlobalRegionManager().allows(DefaultFlag.PISTONS, event.getRetractLocation())) || !(plugin.getGlobalRegionManager().allows(DefaultFlag.PISTONS, event.getBlock().getLocation()))) {
				event.setCancelled(true);
				return;
			}
		}
	...
	public void onBlockExp(BlockExpEvent event) {
		ConfigurationManager cfg = plugin.getGlobalStateManager();
		WorldConfiguration wcfg = cfg.get(event.getBlock().getWorld());
		if (wcfg.disableExpDrops || !plugin.getGlobalRegionManager().allows(DefaultFlag.EXP_DROPS, event.getBlock().getLocation())) {
