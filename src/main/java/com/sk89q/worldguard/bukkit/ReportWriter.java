...
public class ReportWriter {
	...
	public ReportWriter(WorldGuardPlugin plugin) {
		...
		appendWorldConfigurations(plugin, plugin.getServer().getWorlds(), plugin.getGlobalRegionManager(), plugin.getGlobalStateManager());
		appendln("-------------");
	...
	private void appendPluginInformation(Plugin[] plugins) {
		...
		/*
		 * appendHeader("Plugin Information"); log = new LogListBlock(); for (Plugin plugin : plugins) {
		 * log.putChild(plugin.getDescription().getName()) .put("Data folder", plugin.getDataFolder())
		 * .put("Website", plugin.getDescription().getWebsite()) .put("Entry point",
		 * plugin.getDescription().getMain()); } append(log); appendln();
		 */
	}
	...
	private void appendWorldInformation(List<World> worlds) {
		...
		for (World world : worlds) {
			...
			Map<Class<? extends Entity>, Integer> entityCounts = new HashMap<Class<? extends Entity>, Integer>();
