...
/** This processes per-player state information and is also meant to be used as a scheduled task.
 * 
 * @author sk89q */
public class FlagStateManager implements Runnable {
	...
	public void run() {
		...
		for (Player player : players) {
			...
			if (!RegionQueryUtil.isInvincible(plugin, player, applicable) && !plugin.getGlobalStateManager().hasGodMode(player) && !(player.getGameMode() == GameMode.CREATIVE)) {
	...
	private void processHeal(ApplicableRegionSet applicable, Player player, PlayerFlagState state) {
		...
		if (healAmount == null || healDelay == null || healAmount == 0 || healDelay < 0) return;
		if (minHealth == null) {
			minHealth = 0.0;
		}
		if (maxHealth == null) {
			maxHealth = (double) player.getMaxHealth();
		}
		...
		if (player.getHealth() >= maxHealth && healAmount > 0) return;
		
		if (healDelay <= 0) {
			player.setHealth(healAmount > 0 ? maxHealth.intValue() : minHealth.intValue()); // this will
																																											// insta-kill
																																											// if the flag
																																											// is unset
			state.lastHeal = now;
			...
			player.setHealth(Math.min(maxHealth.intValue(), Math.max(minHealth.intValue(), player.getHealth() + healAmount)));
			state.lastHeal = now;
		}
	}
	...
	private void processFeed(ApplicableRegionSet applicable, Player player, PlayerFlagState state) {
		...
		if (feedAmount == null || feedDelay == null || feedAmount == 0 || feedDelay < 0) return;
		if (minHunger == null) {
			minHunger = 0;
		}
		if (maxHunger == null) {
		...
		if (player.getFoodLevel() >= maxHunger && feedAmount > 0) return;
	...
	/** Get a player's flag state. A new state will be created if there is no existing state for the
	 * player.
	 * 
