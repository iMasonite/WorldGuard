...
public class RegionQueryUtil {
	...
	public static Boolean isAllowedInvinciblity(WorldGuardPlugin plugin, Player player) {
		...
		StateFlag.State regionState = plugin.getGlobalRegionManager().get(world).getApplicableRegions(vec).getFlag(DefaultFlag.INVINCIBILITY, plugin.wrapPlayer(player));
		if (regionState == StateFlag.State.ALLOW) return Boolean.TRUE;
		else if (regionState == StateFlag.State.DENY) return Boolean.FALSE;
		else return null;
	}
}
