...
public class WorldGuardPaintingListener implements Listener {
	...
	public void onPaintingBreak(PaintingBreakEvent breakEvent) {
		if (!(breakEvent instanceof PaintingBreakByEntityEvent)) return;
		...
		if (event.getRemover() instanceof Player) {
			...
			if (wcfg.getBlacklist() != null) {
				if (!wcfg.getBlacklist().check(new BlockBreakBlacklistEvent(plugin.wrapPlayer(player), toVector(player.getLocation()), ItemID.PAINTING), false, false)) {
			...
			if (wcfg.blockEntityPaintingDestroy || (wcfg.useRegions && !plugin.getGlobalRegionManager().allows(DefaultFlag.ENTITY_PAINTING_DESTROY, painting.getLocation()))) {
				event.setCancelled(true);
			}
		}
	}
	...
	public void onPaintingPlace(PaintingPlaceEvent event) {
		...
		if (wcfg.getBlacklist() != null) {
			...
			if (!wcfg.getBlacklist().check(new ItemUseBlacklistEvent(plugin.wrapPlayer(player), toVector(player.getLocation()), ItemID.PAINTING), false, false)) {
				event.setCancelled(true);
				return;
			}
		}
	...
	public void onEntityInteract(PlayerInteractEntityEvent event) {
		...
		if (wcfg.useRegions && entity instanceof Painting) {
			...
			if (entity instanceof Painting && ((!plugin.getGlobalRegionManager().allows(DefaultFlag.ENTITY_PAINTING_DESTROY, entity.getLocation())))) {
				event.setCancelled(true);
			}
		}
	}
