...
public class BlacklistEntry {
	...
	public boolean shouldIgnore(LocalPlayer player) {
		if (ignoreGroups != null) {
			for (String group : player.getGroups()) {
				if (ignoreGroups.contains(group.toLowerCase())) return true;
			}
		}
	...
	private String[] getActions(BlacklistEvent event) {
		if (event instanceof BlockBreakBlacklistEvent) return breakActions;
		else if (event instanceof BlockPlaceBlacklistEvent) return placeActions;
		else if (event instanceof BlockInteractBlacklistEvent) return interactActions;
		else if (event instanceof DestroyWithBlacklistEvent) return destroyWithActions;
		else if (event instanceof ItemAcquireBlacklistEvent) return acquireActions;
		else if (event instanceof ItemDropBlacklistEvent) return dropActions;
		else if (event instanceof ItemUseBlacklistEvent) return useActions;
		else return null;
	}
	...
	public boolean check(boolean useAsWhitelist, BlacklistEvent event, boolean forceRepeat, boolean silent) {
		...
		if (shouldIgnore(player)) return true;
		...
		for (String action : actions) {
			...
			if (action.equalsIgnoreCase("deny")) {
				...
				if (action.equalsIgnoreCase("notify")) {
					...
					if (this.message != null) {
						player.printRaw(ChatColor.YELLOW + String.format(message, getFriendlyItemName(event.getType())) + ".");
					}
					else {
